...
export class DosboxSession {
  ...
  async launch(
    ...
    const { extraArgs, wait = true, timeout = 60, dosboxBin: preferredBin, env } = options

    const dosboxBin = resolveDosboxBinary(preferredBin)
    if (!dosboxBin) {
      throw new Error("DOSBox-X binary not found. Set DOSBOX_X_BIN or install dosbox-x in PATH.")
    }
