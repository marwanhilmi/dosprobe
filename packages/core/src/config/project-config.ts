...
export function validateConfig(raw: unknown, filePath: string): ProjectConfig {
  ...
  if (obj["game"] !== undefined) {
    ...
    if (game["exe"] !== undefined) {
      if (typeof game["exe"] !== "string")
        throw new Error(`${filePath}: "game.exe" must be a string`)
      config.game.exe = game["exe"]
    }
    if (game["iso"] !== undefined) {
      if (typeof game["iso"] !== "string")
        throw new Error(`${filePath}: "game.iso" must be a string`)
  ...
  if (obj["qemu"] !== undefined) {
    ...
    if (qemu["ram"] !== undefined) {
      if (typeof qemu["ram"] !== "number" || qemu["ram"] <= 0)
        throw new Error(`${filePath}: "qemu.ram" must be a positive number`)
    ...
    if (qemu["audio"] !== undefined) {
      if (typeof qemu["audio"] !== "boolean")
        throw new Error(`${filePath}: "qemu.audio" must be a boolean`)
      config.qemu.audio = qemu["audio"]
    }
    if (qemu["gdbPort"] !== undefined) {
      if (typeof qemu["gdbPort"] !== "number" || qemu["gdbPort"] <= 0)
        throw new Error(`${filePath}: "qemu.gdbPort" must be a positive number`)
      config.qemu.gdbPort = qemu["gdbPort"]
    }
    if (qemu["accel"] !== undefined) {
      if (typeof qemu["accel"] !== "string" || qemu["accel"].trim().length === 0)
        throw new Error(`${filePath}: "qemu.accel" must be a non-empty string`)
      config.qemu.accel = qemu["accel"]
    }
    if (qemu["cpu"] !== undefined) {
      if (typeof qemu["cpu"] !== "string" || qemu["cpu"].trim().length === 0)
        throw new Error(`${filePath}: "qemu.cpu" must be a non-empty string`)
      config.qemu.cpu = qemu["cpu"]
    }
    if (qemu["smp"] !== undefined) {
      if (typeof qemu["smp"] !== "number" || qemu["smp"] <= 0 || !Number.isFinite(qemu["smp"]))
        throw new Error(`${filePath}: "qemu.smp" must be a positive number`)
  ...
  if (obj["dosbox"] !== undefined) {
    if (
      typeof obj["dosbox"] !== "object" ||
      obj["dosbox"] === null ||
      Array.isArray(obj["dosbox"])
    ) {
    ...
    if (dosbox["binary"] !== undefined) {
      if (typeof dosbox["binary"] !== "string")
        throw new Error(`${filePath}: "dosbox.binary" must be a string`)
      config.dosbox.binary = dosbox["binary"]
    }
    if (dosbox["renderer"] !== undefined) {
      if (typeof dosbox["renderer"] !== "string")
        throw new Error(`${filePath}: "dosbox.renderer" must be a string`)
      config.dosbox.renderer = dosbox["renderer"]
    }
    if (dosbox["memory"] !== undefined) {
      if (typeof dosbox["memory"] !== "number" || dosbox["memory"] <= 0)
        throw new Error(`${filePath}: "dosbox.memory" must be a positive number`)
      config.dosbox.memory = dosbox["memory"]
    }
    if (dosbox["machine"] !== undefined) {
      if (typeof dosbox["machine"] !== "string")
        throw new Error(`${filePath}: "dosbox.machine" must be a string`)
      config.dosbox.machine = dosbox["machine"]
    }
    if (dosbox["cpuCycles"] !== undefined) {
      if (typeof dosbox["cpuCycles"] !== "string")
        throw new Error(`${filePath}: "dosbox.cpuCycles" must be a string`)
  ...
  if (obj["capture"] !== undefined) {
    ...
    if (capture["timeout"] !== undefined) {
      if (typeof capture["timeout"] !== "number" || capture["timeout"] <= 0)
        throw new Error(`${filePath}: "capture.timeout" must be a positive number`)
      config.capture.timeout = capture["timeout"]
    }
    if (capture["waitTime"] !== undefined) {
      if (typeof capture["waitTime"] !== "number" || capture["waitTime"] < 0)
        throw new Error(`${filePath}: "capture.waitTime" must be a non-negative number`)
      config.capture.waitTime = capture["waitTime"]
    }
    if (capture["keyDelay"] !== undefined) {
      if (typeof capture["keyDelay"] !== "number" || capture["keyDelay"] < 0)
        throw new Error(`${filePath}: "capture.keyDelay" must be a non-negative number`)
  ...
  if (obj["server"] !== undefined) {
    ...
    if (server["port"] !== undefined) {
      if (typeof server["port"] !== "number" || server["port"] <= 0)
        throw new Error(`${filePath}: "server.port" must be a positive number`)
