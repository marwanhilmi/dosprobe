...
export class QemuBackend extends Backend {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt === maxAttempts) {
          throw new Error(
            `Failed to connect ${label} after ${maxAttempts} attempts: ${err instanceof Error ? err.message : err}`,
          )
        }
  ...
  async listBreakpoints(): Promise<Breakpoint[]> {
    return this.runExclusive(async () => Array.from(this.breakpoints.values()).map((e) => e.bp))
  }
  ...
  getQmp(): QmpClient | null {
    return this.qmp
  }
  getGdb(): GdbClient | null {
    return this.gdb
  }
