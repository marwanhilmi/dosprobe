...
captureRoutes.get("/captures", async (c) => {
  ...
  for (const file of files) {
    ...
    const match = file.match(
      /^(.+?)_(framebuffer\.bin|screenshot\.\w+|registers\.json|checksums\.json|.+)$/,
    )
    ...
    const type = match?.[2]?.startsWith("framebuffer")
      ? "framebuffer"
      : match?.[2]?.startsWith("screenshot")
        ? "screenshot"
        : match?.[2]?.startsWith("registers")
          ? "registers"
          : match?.[2]?.startsWith("checksums")
            ? "checksums"
            : "data"
  ...
  const captures = Array.from(prefixMap.values()).sort((a, b) => b.timestamp - a.timestamp)
