...
function summarize(
  samples: number[],
): Omit<BenchmarkResult, "name" | "iterations" | "totalBytes" | "throughputMiBps"> {
...
async function benchmark(
  ...
  if (totalBytes > 0) {
    ...
    result.throughputMiBps =
      stats.totalMs > 0 ? totalBytes / (1024 * 1024) / (stats.totalMs / 1000) : 0
  }
  ...
  handler: async (argv) => {
    ...
    try {
      ...
      if (!argv["skip-bench"]) {
        ...
        report.benchmarks.push(
          await benchmark("readRegisters", iterations, async () => {
        ...
        report.benchmarks.push(
          await benchmark(
            `readMemory ${argv["memory-address"]} (${memorySize} bytes)`,
            iterations,
            async () => {
        ...
        if (includeScreenshot) {
          report.benchmarks.push(
            await benchmark("screenshot", screenshotIterations, async () => {
    ...
    if (report.host.accelerators?.length === 1 && report.host.accelerators[0] === "tcg") {
      report.findings.push(
        "QEMU supports only TCG software emulation on this host; gameplay will be significantly slower than hardware-accelerated virtualization.",
      )
    }
    if (!report.vm.connected) {
      report.findings.push(
        "Could not connect to a running QEMU instance via QMP/GDB; runtime microbenchmarks were skipped.",
      )
    }
    if (report.benchmarks.length > 0) {
      ...
      if (memoryBench && memoryBench.p95Ms > 100) {
        report.findings.push(
          `Memory read p95 is high (${memoryBench.p95Ms.toFixed(2)} ms), which can make memory-watch and capture operations feel sluggish.`,
        )
      }
    }
    ...
    if (report.benchmarks.length > 0) {
      ...
      for (const bench of report.benchmarks) {
        ...
        console.log(
          `    avg: ${formatMs(bench.avgMs)} | p95: ${formatMs(bench.p95Ms)} | min/max: ${formatMs(bench.minMs)} / ${formatMs(bench.maxMs)}`,
        )
        ...
        if (bench.totalBytes !== undefined && bench.throughputMiBps !== undefined) {
          console.log(
            `    throughput: ${bench.throughputMiBps.toFixed(2)} MiB/s (${bench.totalBytes} bytes total)`,
          )
        }
      }
    }
