...
async function setupQemu(
  ...
  if (existsSync(paths.diskImage)) {
    ...
    try {
      ...
      execSync(
        `sed '/^if not exist SETUP.BAT/,/^:Done/{ /^:Done/!d; }' "${tmpBat}" > "${tmpBat}.fixed"`,
      )
      ...
      execSync(
        `mcopy -o -i "${imgFile}"@@${offset} "${ctmouseDir}/BIN/CTMOUSE.EXE" ::FREEDOS/BIN/CTMOUSE.EXE`,
      )
      ...
      execSync(`qemu-img convert -f raw -O qcow2 "${imgFile}" "${paths.diskImage}"`, {
        stdio: "inherit",
      })
      ...
      console.error(
        "Failed to set up FreeDOS disk image:",
...
async function setupDosbox(
  ...
  if (force || !existsSync(defaultConf)) {
    ...
    conf.setAutoexec([`MOUNT C "${paths.driveCPath}"`, "C:", "SET BLASTER=A220 I5 D1 H5 T6"])
