    ...
    try {
      ...
      if (argv.json) {
        ...
        if (result.framebuffer) {
          console.log(
            `  Framebuffer: ${result.framebuffer.length} bytes (${result.checksums.get("framebuffer")})`,
          )
        }
        if (result.screenshot) {
          console.log(
            `  Screenshot: ${result.screenshotFormat} (${result.checksums.get("screenshot")})`,
          )
        }
        if (result.registers) {
          console.log(
            `  Registers: EIP=0x${result.registers.eip.toString(16)} CS=0x${result.registers.cs.toString(16)}`,
          )
        }
